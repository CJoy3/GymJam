...
--
-- NOTE: This is only a tiny handful of demo gyms for local dev. The REAL gym
-- locations shown on the map (every UK branch of the 9 major chains) live in
-- gyms_seed.sql, which is generated by scripts/fetch_uk_gyms.py and prepopulated
-- into Supabase. Load order: schema.sql -> gyms_seed.sql -> (optionally) seed.sql.
