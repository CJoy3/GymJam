...
import {
  Btn, Card, Chip, Eyebrow, FadeInItem, H1, H2, H3, Num, Ring, Stat, Sub,
} from '../ui/components';
                ...
                >
                  <View style={styles.rowBetween}>
                    <View style={[styles.rowGap, { flex: 1 }]}>
                      <View style={[styles.iconChip, { backgroundColor: on ? 'rgba(27,23,20,0.06)' : C.accentSoft }]}>
...
export function Home({ onCheckIn, onPlan, onPot }: { onCheckIn: () => void; onPlan: () => void; onPot: () => void }) {
  const { thisWeek, elo, streak, checkInToday, todayIndex, displayName, potCurrent } = useAppState();
  ...
  const potValue = potCurrent?.total_pot_elo ?? 0;
  const potOnTrack = potCurrent ? potCurrent.members.filter((m) => m.is_on_track).length : 0;
  const potMembers = potCurrent?.members.length ?? 0;
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView refreshControl={refresh} contentContainerStyle={pageWrap} showsVerticalScrollIndicator={false}>
          ...
          <Card padding={SPACE.xl}>
            <View style={[styles.rowBetween, { marginBottom: 16 }]}>
              <View>
                ...
                <H2 style={{ marginTop: 4 }}>Your pledge</H2>
              </View>
        ...
        {/* Pot card — cream tone draws the eye */}
          ...
          <Card onPress={onPot} tone="cream" padding={SPACE.xl}>
            <View style={styles.rowBetween}>
              <View>
                ...
                <Text style={[styles.potValue, { color: C.primaryFg }]}>
                  {potValue.toLocaleString()} <Text style={[styles.potUnit, { color: C.primaryFg }]}>ELO</Text>
                </Text>
              </View>
            ...
            <Text style={[styles.subOnCream, { marginTop: 6 }]}>
              {potMembers > 0
                ? `${potOnTrack} of ${potMembers} ${potMembers === 1 ? 'member' : 'members'} on track`
...
/* ╭─────────────────────────────────────────────────────────╮
   │ Plan next week — local-state day picker + setter editor │
   ╰─────────────────────────────────────────────────────────╯ */
...
export function PlanWeek({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  const commit = async () => {
    ...
    setSaving(true);
    try {
      const days = local.map((s, i) => (s === 'planned' ? i : -1)).filter((i): i is number => i !== -1);
          ...
          <Card padding={SPACE.lg}>
            <View style={styles.rowGap}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Sub style={{ marginTop: 2 }}>Change your pledge until then.</Sub>
              </View>
            </View>
          </Card>
        </FadeInItem>
                ...
                <Card padding={SPACE.lg} style={isJoined ? { borderColor: C.primary, borderWidth: 1.5 } : undefined}>
                  <View style={styles.rowGap}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <View style={[styles.rowGap, { gap: 8, marginTop: 6 }]}>
...
const styles = StyleSheet.create({
  ...
  potUnit: { fontFamily: FONT.semibold, fontSize: 16 },
  creamArrow: {
